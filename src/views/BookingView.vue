<template>
    <div class="venues-page">
      <div class="container mt-4">
        ...
        <select v-model="filterOption" class="form-select mb-4" aria-label="Filter venues">
          <option value="all">All Venues</option>
          <option value="available">Available Venues</option>
        </select>
        <div class="row justify-content-center">
          <div class="col-md-3 col-sm-6 mb-4" v-for="venue in filteredVenues" :key="venue.venue_id">
            <div class="venue-card">
              ...
              <div class="workshop-card mb-2" v-for="workshop in venue.workshops" :key="workshop.workshop_id">
                <h6 class="workshop-date">{{ formattedDate(workshop.workshop_date) }}</h6>
                <p v-if="workshop.seats_available > 0" class="seats-available">Seats Available: {{ workshop.seats_available }}</p>
                <p v-else class="sold-out">Sold Out</p>
                <button v-if="workshop.seats_available > 0" class="book-btn" @click="showBookingForm(workshop)">Book Now</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="showForm" class="booking-form mx-auto">
          ...
          <form>
            <div class="mb-3">
              ...
              <input type="text" class="form-control" id="name" v-model="bookingDetails.name" required>
            </div>
            <button @click.prevent="submitBooking()" class="confirm-btn">Confirm Booking</button>
            <button @click.prevent="showForm = false" class="cancel-btn">Cancel</button>
          </form>
        </div>
      </div>
    </div>
      ...
      filteredVenues() {
        if (this.filterOption === 'available') {
          return this.venues.filter(venue =>
            ...
            ...venue,
            workshops: venue.workshops.filter(workshop => workshop.seats_available > 0)
          }));
        }
        return this.venues;
      },
